...
def get_dataloaders(folder='flower_data', batch_size=32):
    ...
    Args:
        folder: String with the folder name that contains the train
        and test folders.
    """
    ...
    valid_data = torchvision.datasets.ImageFolder(valid_dir,
                                                  transform=data_transforms)
...
def prepare_model(model, n_out):
    ...
    # Modify Classifier
    try:
        # Looks for las_layer_attr
        n_inputs = getattr(model, model.last_layer_attr).in_features
    except AttributeError:
        n_inputs = getattr(model, model.last_layer_attr)[-1].in_features
    attr = getattr(model, model.last_layer_attr)
    # Check if returns a Sequential
...
def train_model(model, loaders, train_on_gpu=False,
    ...
    Args:
        ...
        loaders: Dictionary with the keys 'train' and 'test' for training and
                 validation data.
    ...
    if optimizer == 'SGD':
        optimizer = optim.SGD(getattr(model,
                                      model.last_layer_attr).parameters(),
                              lr=lr)
    else:
        optimizer = optim.Adam(getattr(model,
                                       model.last_layer_attr).parameters(),
                               lr=lr)
        ...
        for batch_i, (data, target) in enumerate(loaders['train']):
            ...
            if (batch_i + 1) % print_every == 0:
                ...
                print(f"Epoch {epoch}/{n_epochs}.. "
                      f"Train loss: {train_loss / (print_every *
                                     data.size()[0]): .3f}.. "
                      f"Test loss: {test_loss / len(loaders['test']):.3f}.. "
                      f"Accuracy: {correct.sum()/len(correct) * 100:.2f} % " +
